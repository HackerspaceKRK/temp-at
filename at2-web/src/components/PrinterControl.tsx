...
import { Box, Bell, BellRing, Thermometer, Layers, Clock, FileText, Play, CheckCircle2, XCircle, AlertTriangle } from "lucide-react";
        ...
        <div className="flex flex-col gap-3">
          ...
          {state && (
            <div className="flex flex-col gap-1.5 text-sm">
              ...
              {state.print_error && state.print_error !== "00000000" && (
                <div className="flex items-start gap-2 text-xs text-destructive">
                  <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                  <span>
                    {state.print_error_text || t("Error code")}
                    <span className="opacity-70"> ({state.print_error})</span>
                  </span>
                </div>
              )}
