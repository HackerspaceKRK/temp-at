...
// SetRelay publishes ON/OFF state for a virtual relay device.
// This is a best-effort generic implementation: For multi-endpoint devices, Zigbee2MQTT
// often expects keys like state_l1, state_l2, ... tied to endpoint numbers.
// We attempt to map based on endpoint and relay index extracted from the virtual name.
//
// NOTE: Without full schema knowledge, this may need refinement.
func (a *MQTTAdapter) SetRelay(virtualName string, on bool) error {
	// Find corresponding virtual device.
	a.virtualMu.RLock()
	var vd *VirtualDevice
	for i := range a.virtualDevices {
		if a.virtualDevices[i].Name == virtualName {
			vd = &a.virtualDevices[i]
			break
		}
	}
	a.virtualMu.RUnlock()
	...
	if vd == nil {
		return fmt.Errorf("unknown virtual device %q", virtualName)
	}
	if vd.Type != "relay" {
		return fmt.Errorf("virtual device %q is not a relay", virtualName)
	}
	if a.client == nil || !a.client.IsConnectionOpen() {
		return errors.New("mqtt not connected")
	}

	// Determine state key. If endpoint exists and relay index is in name, prefer "state_lX".
	stateVal := "OFF"
	if on {
		stateVal = "ON"
	}
	...
	relayIdx := relayIndexFromVirtualName(virtualName)
	if relayIdx > 0 {
		key := fmt.Sprintf("state_l%d", relayIdx)
		payload = map[string]string{key: stateVal}
	} else {
		// Single relay fallback.
		payload = map[string]string{"state": stateVal}
	}

	data, _ := json.Marshal(payload)
	token := a.client.Publish(deviceStateSetTopic(vd.BaseName), 0, false, data)
	if !token.WaitTimeout(5 * time.Second) {
...
// relayIndexFromVirtualName extracts the integer after '/l' if present (e.g. name ending with /l2).
func relayIndexFromVirtualName(name string) int {
